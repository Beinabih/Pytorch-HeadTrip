...
class Dreamer:
    def __init__(self, img_p, outpath, config):
        ...
        if self.no_class:
            self.model = nn.Sequential(
                    *self.layers[: (self.at_layer_para + 1)])

        print(self.config["num_iterations"], self.octave_scale,
              self.config["num_octaves"], self.lr)
    ...
    def deep_dream(self, image, model, i, seq, mask=None):
        ...
        octaves = get_octaves(
            image_p, self.config["num_octaves"], self.octave_scale)
        ...
        if self.depth:
            ...
            d_img = np.expand_dims(d_img, 0)
            d_img_octaves = get_octaves(d_img, self.config["num_octaves"], self.octave_scale)
        ...
        for octave, octave_base in enumerate(tqdm.tqdm(octaves[::-1], desc="Dreaming")):
            if octave > 0:
                ...
                self.detail = nd.zoom(self.detail, np.array(
                    octave_base.shape) / np.array(self.detail.shape), order=1)
            ...
            if mask is not None:
                ...
                dreamed_image = self.dream(
                    input_image, model, d_img_octaves[octave])
    ...
    def random_para(self):
        ...
        self.lr = random.choice(
                    [0.01, 0.009, 0.008, 0.02, 0.03, 0.007])
    ...
    def dream_single(self):
        ...
        for i, path in enumerate(self.img_list):
            ...
            d_img = self.deep_dream(self.transform(
                    img1), self.model, i, seq="first")
        ...
        for i, path in enumerate(self.img_list[self.config["start_position"]:]):
            ...
            if i > 0:
                ...
                background_masked[background_masked >
                                  0] = 1 - (flow_iter * 0.1)  # 0.5
                background_masked[background_masked == 0] = flow_iter * 0.1

                d_img = self.deep_dream(
                    self.transform(np.uint8(feature_img)), self.model, i, seq="first", mask=background_masked
                )
